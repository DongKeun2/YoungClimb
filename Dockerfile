...
WORKDIR /var/jenkins_home/workspace/YoungClimb/{react repo이름}
COPY package*.json ./
RUN npm install
COPY . .
RUN npm run build
FROM nginx:stable-alpine as production-stage
COPY --from=build-stage /var/jenkins_home/workspace/YoungClimb/{react repo이름}/dist /usr/share/nginx/html
