...
export default function StoreDetail({route, navigation}) {
  ...
  useEffect(() => {
    ...
    axios
      .get(api.center(route.params.Id), {cancelToken: source.token})
      .then(res => {
  ...
  useEffect(() => {
    ...
    const todayInfo = detailInfo.centerTimeList.filter(
      info => info.day === day,
    )[0];
    if (todayInfo) {
      setTodayTimeInfo(
        `${dayDict[day]}  ${todayInfo.timeStart.slice(
          0,
          5,
        )} - ${todayInfo.timeEnd.slice(0, 5)}`,
      );
    }
  }, [detailInfo]);
    ...
    <>
      ...
      {isLoading ? (
          ...
          <View style={styles.mainContainer}>
            <View style={styles.infoFlex}>
              ...
              <View style={{width: 27, alignItems: 'center'}}>
                <Text style={{color: 'black', fontSize: 12.5}}>|</Text>
              </View>
              <View
                style={{
                  height: 10,
        ...
        <View style={{alignItems: 'center', height: '100%', width: '100%'}}>
          <Text>지점 정보가 존재하지 않습니다.</Text>
        </View>
      ) : (
        <ScrollView
          showsVerticalScrollIndicator={false}
          style={{width: '100%', backgroundColor: 'white', paddingBottom: 50}}>
          <Image style={styles.image} source={{uri: detailInfo?.img}} />
          ...
          <View style={styles.mainContainer}>
            ...
            <View style={styles.infoFlex}>
              ...
              <TouchableOpacity
                onPress={() => {
                  Linking.openURL(`tel:${detailInfo.phoneNumber}`);
                }}>
                <Text style={styles.phone}>{detailInfo.phoneNumber}</Text>
              </TouchableOpacity>
            </View>
            {/* 운영시간 text */}
            <View style={styles.infoFlex}>
                ...
                isTimeToggleOpen ? (
                  <>
                      ...
                      {detailInfo.centerTimeList.map((info, idx) => {
                        return (
                          <Text
                            ...
                            {dayDict[info.day]} {info.timeStart.slice(0, 5)} -{' '}
                            {info.timeEnd.slice(0, 5)}
                          </Text>
                        );
                      })}
                    </View>
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        setIsTimeToggleOpen(!isTimeToggleOpen);
                      }}>
                ...
                isPriceToggleOpen ? (
                  <>
                    <View>
                      {detailInfo.centerPriceList.map((info, idx) => {
                        ...
                        const priceFormat = p
                          ...
                          .replace(/\B(?=(\d{3})+(?!\d))/g, ',');
                        return (
                          <Text key={idx + 'price'} style={styles.detailfont}>
                    ...
                    <TouchableOpacity
                      ...
                      style={styles.toggleBtn}
                      onPress={() => {
                        setIsPriceToggleOpen(!isPriceToggleOpen);
                      }}>
                      <ExpandUp style={{marginTop: 6}} />
                    </TouchableOpacity>
                  </>
                  ...
                  <>
                    <TouchableOpacity
                      ...
                      <Text style={styles.detailfont}>
                        {detailInfo.centerPriceList[0].name} ꞏ{' '}
                        {detailInfo.centerPriceList[0].price
                          .toString()
                          .replace(/\B(?=(\d{3})+(?!\d))/g, ',')}{' '}
                        원{' '}
                      </Text>
                ...
                return (
                  <View
                    ...
                    style={{
                      ...
                      height: 25,
                      width: `${
                        100 / detailInfo.centerLevelList.length - 0.2
                      }%`,
                      backgroundColor: levelColorDict[item.color],
                    }}
                  />
                );
              })}
